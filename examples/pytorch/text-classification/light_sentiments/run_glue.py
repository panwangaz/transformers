...
from utils.fit_function_predict import get_recall_2_precision_f1
...
def main():
    ...
    def my_metrics(pred_labels, true_labels, probs, group_id_list): 
        global BEST_PR_AUC_LOG_PATH
        id2label = {0: "negative", 1: "positive"}
        # probabilities = probs[:,1]
        probabilities = probs[:,1] - probs[:,0]
        roc_auc = roc_auc_score(true_labels, probabilities)
        precisions, recalls, _ = precision_recall_curve(true_labels, probabilities)
        ...
        metric_dict = {"TP": 0, "FP":0, "FN":0} # avoid devide zero error 
        ...
        metric_dict["max_f1"] = max_f1
        with open(BEST_PR_AUC_LOG_PATH, "r") as jf:
            best_pr_auc_bf_dict = json.load(jf)
        if pr_auc > best_pr_auc_bf_dict["best_pr_auc"]:
            best_pr_auc_bf_dict["best_pr_auc"] = pr_auc
            best_pr_auc_bf_dict["roc_auc"] = roc_auc
            best_pr_auc_bf_dict["recalls"] = recalls.tolist()
            best_pr_auc_bf_dict["precisions"]  = precisions.tolist()
            with open(BEST_PR_AUC_LOG_PATH, "w") as fout:
                json.dump(best_pr_auc_bf_dict, fout)
    ...
    import datetime
    global BEST_PR_AUC_LOG_PATH
    BEST_PR_AUC_LOG_PATH = "./tmp/best_pr_auc_metrics/" + datetime.datetime.now().strftime("%Y_%m_%d-%H_%M_%S") + ".json"
    ...
    print("best pr auc: {}".format(best_pr_auc_bf_dict["best_pr_auc"]))
    print("the roc auc here: {}".format(best_pr_auc_bf_dict["roc_auc"]))
    get_recall_2_precision_f1(best_pr_auc_bf_dict)
