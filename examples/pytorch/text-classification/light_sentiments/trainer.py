...
class Trainer:
    ...
    def predict(
        ...
        output,hidden_states = eval_loop(
            test_dataloader,
        ...
        return PredictionOutput(predictions=output.predictions, label_ids=output.label_ids, metrics=output.metrics),hidden_states
    ...
    def prediction_loop(
        ...
        hidden_states_host: Union[torch.Tensor, List[torch.Tensor]] = None
        ...
        if not prediction_loss_only:
            ...
            hidden_states_gatherer = DistributedTensorGatherer(world_size, num_examples, make_multiple_of=make_multiple_of)
        ...
        for step, inputs in enumerate(dataloader):
            loss, logits, labels, hidden_states = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            inputs_decode = self._prepare_input(inputs["input_ids"]) if args.include_inputs_for_metrics else None
            ...
            if logits is not None:
                try:
                    ...
                    preds_host = logits if preds_host is None else nested_concat(preds_host, logits.unsqueeze(0), padding_index=-100)
            if labels is not None:
                try:
                    labels_host = labels if labels_host is None else nested_concat(labels_host, labels, padding_index=-100)
                except:
                    labels_host = labels if labels_host is None else nested_concat(labels_host, labels.unsqueeze(0), padding_index=-100)
            if hidden_states is not None:
                try:
                    hidden_states_host = hidden_states if hidden_states_host is None else nested_concat(hidden_states_host, hidden_states, padding_index=-100)
                except:
                    hidden_states_host = hidden_states if hidden_states_host is None else nested_concat(hidden_states_host, hidden_states.unsqueeze(0), padding_index=-100)
            ...
            if args.eval_accumulation_steps is not None and (step + 1) % args.eval_accumulation_steps == 0:
                ...
                if not prediction_loss_only:
                    ...
                    hidden_states_gather.add_arrays(self._gather_and_numpify(hidden_states_host, "eval_hidden_states"))

                # Set back to None to begin a new accumulation
                losses_host, preds_host, labels_host, inputs_host, hidden_states_host = None, None, None, None, None
        ...
        if not prediction_loss_only:
            ...
            labels_gatherer.add_arrays(self._gather_and_numpify(labels_host, "eval_label_ids"))
            inputs_gatherer.add_arrays(self._gather_and_numpify(inputs_host, "eval_inputs_ids"))
            hidden_states_gatherer.add_arrays(self._gather_and_numpify(hidden_states_host, "eval_hidden_states"))

        eval_loss = eval_losses_gatherer.finalize()
        preds = preds_gatherer.finalize() if not prediction_loss_only else None
        label_ids = labels_gatherer.finalize() if not prediction_loss_only else None
        inputs_ids = inputs_gatherer.finalize() if not prediction_loss_only else None
        hidden_states = hidden_states_gatherer.finalize() if not prediction_loss_only else None
        ...
        return EvalLoopOutput(predictions=preds, label_ids=label_ids, metrics=metrics, num_samples=num_examples), hidden_states
